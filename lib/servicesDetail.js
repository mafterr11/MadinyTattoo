...
export const serviceDetails = [
  {
    ...
    priceNote:
      "Prețul final depinde de dimensiune, complexitate și zona aleasă.",
    // Condensed version of the policy that has its own section on the
    // dedicated page — the figures come from lib/deposit.js so the two can
    // never disagree.
    notice: {
      title: "Politica de avans",
      body: `Programarea se confirmă cu un avans între ${deposit.min} și ${deposit.max} lei, în funcție de costul total al tatuajului.`,
      items: [
        "Avansul nu se returnează dacă nu te prezinți la programare.",
        `Reprogramarea se anunță cu cel puțin ${deposit.noticeHours} de ore înainte, altfel avansul se pierde.`,
      ],
      linkLabel: "Citește politica completă",
      linkHref: `/servicii/tatuaje#${deposit.anchor}`,
    },
    ctaLabel: "Mai multe informații",
  },
  {
    key: "micropigmentare",
  ...
  },
  {
    ...
    // The procedure itself, not a portrait of the operator: showing the
    // handpiece over skin answers "what happens to me" at a glance.
    image: "/gallery/laserService/laserRemoval.webp",
    imageAlt:
      "Ședință de eliminare a unui tatuaj cu laser la Madiny Tattoo București",
    intro:
    ...
    priceGroups: [
      {
        ...
        tiers: [{ label: "Laser sprâncene", price: "300 RON" }],
      },
      {
...
/** Lookup by key, so a page can pull its own copy without re-declaring it. */
export const getService = (key) =>
  serviceDetails.find((service) => service.key === key);
