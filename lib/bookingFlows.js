 ...
 *
 * `label` is only passed where the question is not the one the step's title
 * already asks — on the tattoo flow the artist comes first, so the area needs
 * a heading of its own.
 */
const areaField = (summaryLabel, label) => ({
  name: "area",
  type: "choice",
  ...(label ? { label } : {}),
  options: BODY_AREAS,
...
export const bookingFlows = [
  {
    ...
    intent: "un tatuaj",
    detailsTitle: "Cu cine vrei să lucrezi?",
    detailsDescription:
      "Alege artista, apoi zona și o dimensiune estimativă. Nimic nu e bătut în cuie — stabilim detaliile împreună.",
    fields: [
      // Tattoos only: the studio's other three services are done by whoever is
      // free, and offering a choice that does not exist would be a promise the
      // booking cannot keep.
      {
        name: "artist",
        type: "artist",
        options: [...artists.map((artist) => artist.name), ANY_ARTIST.name],
        ariaLabel: "Artista care face tatuajul",
        required: true,
        summaryLabel: "Artistă",
        emoji: EMOJI.artist,
      },
      areaField("Zona corpului", "Zona corpului"),
      {
        name: "size",
        type: "range",
